...
import { useFetchRegisterById } from "@/app/sign-up/hooks";
...
const BeasiswaPage = ({ params }: SignUpIdProps) => {
  const beasiswa = useFetchRegisterById();
  useEffect(() => {
    async function fetchData() {
      await beasiswa.mutate(params.beasiswaId);
    }
  ...
  return (
    <div className="relative flex flex-col justify-center items-center min-h-screen overflow-hidden px-4">
      <div className="w-full mb-12 mt-12 m-auto lg:max-w-lg">
        <Card>
          <CardHeader>
            <CardTitle className="text-center">Data Beasiswa</CardTitle>
          </CardHeader>
